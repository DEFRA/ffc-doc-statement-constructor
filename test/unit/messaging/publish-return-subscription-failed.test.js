...
describe('publishReturnSubscriptionFailed', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks()
    config.returnSubscriptionFailed = 'test-queue'
    console.log = jest.fn()
  })
  ...
  test('should handle sendMessage error', async () => {
    ...
    await expect(publishReturnSubscriptionFailed(paymentRequest, error)).rejects.toThrow('Send message failed')
  })
})
